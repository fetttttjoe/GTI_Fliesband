...
//unsigned char setMotorVal(DSCB boardHandle,PtrOutVector_PosAnlage outV);
...
int main(void)
{
    ...
    //Infinite Loop 
    while(1){
        if(getModelButtons(boardHandle, &InV) == 0){
            break;
        }
        if(getModelSensors(boardHandle, &InV)==0){
           break;
        }
